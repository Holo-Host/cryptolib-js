...
class KeyManager {
    ...
    sign (message) {
        ...
        return this.#ed.sign(message, this.#private_key)
    }

    verify (message, signature) {
        this.verifyWithPublicKey(signature, message, this.publicKey())
    }
    /**
     * Returns true if the signature was generated by signing the message using the keypair of the pubkey
     *
     * @param {Uint8Array} message - The first number.
     ...
     */
    verifyWithPublicKey (message, signature, pubkey) {
        if(!this.#ed) {
            throw new Error ("Attempt to use ed before initialized")
        }

        return this.#ed.verify(signature, message, pubkey)
    }
}
